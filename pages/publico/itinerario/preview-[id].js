...
export default function ItinerarioPreview() {
  const router = useRouter()

  // Obtener datos del localStorage o sessionStorage para la vista previa
  const [itinerarioData, setItinerarioData] = useState(null)

  React.useEffect(() => {
    // Intentar obtener datos de la vista previa desde sessionStorage
    const previewData = sessionStorage.getItem('itinerarioPreview')
    if (previewData) {
      setItinerarioData(JSON.parse(previewData))
    }
  }, [])
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Vista Previa no disponible</h1>
          <p className="text-gray-600">No hay datos de itinerario para mostrar en la vista previa.</p>
        </div>
      </div>
    )
  }
  ...
  const diasAgrupados = itinerarioData.dias.reduce((acc, dia) => {
    acc[dia.fecha] = {
      ...dia,
...
ItinerarioPreview.getLayout = function getLayout(page) {
  return page
}
